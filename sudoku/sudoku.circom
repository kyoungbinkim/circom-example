pragma circom 2.0.0;

// code from https://github.com/rdi-berkeley/zkp-course-lecture3-code/blob/main/circom/sudoku.circom 

// check (in0 - in1) is nonZero
template NonEqual(){
    signal input in0;
    signal input in1;
    signal inv;
    inv <-- 1/ (in0 - in1);
    inv*(in0 - in1) === 1;
}

// check all elements in array are uique
template Distinct(n) {
    signal input in[n];
    component nonEqual[n][n];
    for(var i = 0; i < n; i++){
        for(var j = 0; j < i; j++){
            // in[i] != in[j] 인지 체크
            nonEqual[i][j] = NonEqual();
            nonEqual[i][j].in0 <== in[i];
            nonEqual[i][j].in1 <== in[j];
        }
    }
}

// Enforce that 0 <= in < 16
template Bits4(){
    signal input in;
    signal bits[4];
    var bitsum = 0;
    for (var i = 0; i < 4; i++) {
        // in의 i번째 bit 
        bits[i] <-- (in >> i) & 1;          // in i개 shift 후 0x0001 과 비트마스킹
        bits[i] * (bits[i] - 1) === 0;      // bits[i] binary check
        bitsum = bitsum + 2 ** i * bits[i]; // bitsum에 누적
    }
    bitsum === in;  // if in >= 16, then in > bitsum
}

// Enforce that 1 <= in <= 9
template OneToNine() {
    signal input in;
    component lowerBound = Bits4();
    component upperBound = Bits4();
    lowerBound.in <== in - 1;
    upperBound.in <== in + 6;
}

// mathch solution array and puzzle array inputs
template MatchSolutionToPuzzle(n) {
    signal input puzzle[n][n];
    signal input solution[n][n]; 

    for(var row=0; row<n; row++){
        for(var col=0; col<n; col++){
            puzzle[row][col] * (puzzle[row][col] - solution[row][col]) === 0;
        }
    }
}

template Sudoku(n) {
    // solution is a 2D array: indices are (row_i, col_i)
    signal input solution[n][n];
    // puzzle is the same, but a zero indicates a blank
    signal input puzzle[n][n];

    component distinct[n];
    component inRange[n][n];
    
    // 솔루션이 퍼즐의 솔수션이 맞는지 체크
    component mstp = MatchSolutionToPuzzle(n);
    mstp.solution <== solution;
    mstp.puzzle <== puzzle;

    // 솔루션이 스도쿠 규칙에 맞는지 체크 (행)
    for (var row_i = 0; row_i < n; row_i++) {
        for (var col_i = 0; col_i < n; col_i++) {
            if (row_i == 0) {
                distinct[col_i] = Distinct(n);
            }
            inRange[row_i][col_i] = OneToNine();
            inRange[row_i][col_i].in <== solution[row_i][col_i];
            distinct[col_i].in[row_i] <== solution[row_i][col_i];
        }
    }
}

component main {public[puzzle]} = Sudoku(9);